...
- Do not create or change code without clear direction that new or changed code is requested. Always assume we are working out an idea until it is clear how new/edit code should be completed.
- Run lint/typecheck without extra flags (`npm run lint`, `npm run typecheck`) so all issues surface, and fix prettier warnings too.
- Default mindset: propose the cleanest long-term approach; refactors are welcome when they improve the foundation. 
- Do no mess with files in src/core/db/migrations. You can read them, but they are auto generated by drizzle and should not be manually edited or created.
